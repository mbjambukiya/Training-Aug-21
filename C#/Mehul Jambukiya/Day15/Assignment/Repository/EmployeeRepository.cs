...
namespace Day15_Assignment.Repository
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public IEnumerable<Employee> GetEmployees(long[] arrayOfEmpId)
        {
            try
            {
                List<Employee> result = new List<Employee>();

                foreach (var item in arrayOfEmpId)
                {
                    if (_context.Employees.Find(item)!=null)
                    {
                        result.Add(_context.Employees.Find(item));
                    }
                }
                return result;
            }
